...
functions[15]['VDL'] = function () {
    ...
    if (R1000.mod(100).eq(0)) {
        xxx = Big(Math.random())
            .times(b.minus(a).plus(1))
            .round(0, Big.roundDown)
            .plus(a)
            .div(10)
            .plus(
                Big(Math.random())
                    .times(Big(9).minus(Big(0)).plus(1))
                    .round(0, Big.roundDown)
                    .plus(Big(0))
                    .div(100),
            )
            .plus(
                Big(Math.random())
                    .times(Big(9).minus(Big(1)).plus(1))
                    .round(0, Big.roundDown)
                    .plus(Big(1))
                    .div(1000),
            )
    ...
    let ANS = 0
    if (R1000.mod(10).eq(0)) {
        ANS = R1000.div(1000).minus(1).times(100).round().toNumber()
    }
    if (R1000.mod(10).gt(0)) {
        ANS = R1000.div(1000).minus(1).times(1000).round().div(10).toNumber()
    }
...
functions[15]['DDL1'] = function () {
    ...
    for (var i = 0; i < N.minus(2).toNumber(); i++) {
        //Формируем массив первых цифр после запятой в долях столбца долг (кроме цифры "0")
        let gonext = true
        while (gonext) {
            let a_candidate =
                DigitsMas[
                    Math.floor(Math.random() * (DigitsMas.length - 1)) + 1
                ]
    ...
    while (podbor_needed) {
        let bMas_candidate = []
        for (var i = 0; i < N.minus(2).toNumber(); i++) {
            bMas_candidate[bMas_candidate.length] =
                DigitsMas[Math.floor(Math.random() * DigitsMas.length)]
        }
        let sum = Big(0)
        bMas_candidate.forEach(function (value) {
            sum = sum.plus(value)
        })
        if (sum.mod(10).eq(0)) {
            podbor_needed = false
    ...
    part +=
        ' года Моне был выдан кредит на развитие бизнеса. В таблице представлен график его погашения.\n'
    part += '\n'
    part +=
        '+-------------------------------+' +
        '-------+'.repeat(N.toNumber()) +
    ...
    part +=
        '%, а выплаты по погашению кредита происходили в первой половине каждого месяца, начиная с февраля. На сколько процентов общая сумма выплат при таких условиях больше суммы самого кредита?'

    //БЛОК вычисления ответа к задаче
    let sum_aMas = Big(0)
    let sum_bMas = Big(0)
    aMas.forEach(function (value) {
        sum_aMas = sum_aMas.plus(value)
    })
    bMas.forEach(function (value) {
        sum_bMas = sum_bMas.plus(value)
    })
    let ANS = R1000.div(10)
        .plus(sum_aMas.times(10).plus(sum_bMas).times(R1000.div(1000).minus(1)))
        .minus(100)
        .toNumber()
...
functions[15]['DDL2'] = function () {
    ...
    let questionVariant =
        questionVariantMas[
            Math.floor(Math.random() * questionVariantMas.length)
        ] //Случайно выбираем, какой вопрос будем ставить [0] для вопроса, когда каждая из выплат больше Xborder, [1] - когда спрашивают, чтобы каждая выплата меньше Xborder
    ...
    for (var i = 1; i < N.minus(2).toNumber(); i++) {
        XMas[i] = R1000.div(1000)
            .times(aMas[i - 1].div(10).plus(bMas[i - 1].div(100)))
            .minus(aMas[i].div(10).plus(bMas[i].div(100)))
    }
    XMas[N.minus(2).toNumber()] = R1000.div(1000).times(
        aMas[N.minus(3).toNumber()]
            .div(10)
            .plus(bMas[N.minus(3).toNumber()].div(100)),
    )
...
functions[15]['DDL3'] = function () {
    ...
    let sumXBorder =
        sumXborderMas[Math.floor(Math.random() * sumXborderMas.length)]
    for (var i = 0; i < N.minus(2).toNumber(); i++) {
        //Формируем массив первых цифр после запятой в долях столбца долг (кроме цифры "0")
        let gonext = true
        while (gonext) {
    ...
    let sumX = Big(0)
    XMas.forEach(function (value) {
        sumX = sumX.plus(value)
    })
    ...
    if (questionVariant == 0) {
        part +=
            'Найдите наименьшее значение S, при котором общая сумма выплат будет больше '
        part += sumXBorder.toString()
        part += ' млн рублей'
    }
    if (questionVariant == 1) {
        part +=
            'Найдите наибольшее значение S, при котором общая сумма выплат будет меньше '
        part += sumXBorder.toString()
        part += ' млн рублей'
    }
...
functions[15]['DDL4'] = function () {
    ...
    let questionVariant =
        questionVariantMas[
            Math.floor(Math.random() * questionVariantMas.length)
        ] //Случайно выбираем, какой вопрос будем ставить: [0] для вопроса, когда сумма выплат больше sumXborder, [1] - когда спрашивают, чтобы сумма выплат меньше sumXborder
    let sumXBorder =
        sumXborderMas[Math.floor(Math.random() * sumXborderMas.length)]
    for (var i = 0; i < N.minus(2).toNumber(); i++) {
        //Формируем массив первых цифр после запятой в долях столбца долг (кроме цифры "0")
    ...
    let sumD = Big(0)
    aMas.forEach(function (value) {
        sumD = sumD.plus(value.div(10))
    })
    bMas.forEach(function (value) {
        sumD = sumD.plus(value.div(100))
    })

    let Smin = Big(1)
        .div(Big(1.29).plus(Big(0.29).times(sumD)))
        .times(sumXBorder)
        .round(0, Big.roundUp)
    let Smax = Big(1)
        .div(Big(1.02).plus(Big(0.02).times(sumD)))
        .times(sumXBorder)
        .round(0, Big.roundDown)
    ...
    for (var i = 0; i < N.minus(2).toNumber(); i++) {
        part +=
            ' ' +
            S.times(aMas[i].div(10).plus(bMas[i].div(100))).toString() +
            ' '.repeat(
                10 -
                    S.times(aMas[i].div(10).plus(bMas[i].div(100))).toString()
                        .length,
    ...
    if (questionVariant == 1) {
        part +=
            'Найдите наибольшее значение r, при котором общая сумма выплат будет меньше '
        part += sumXBorder.toString()
        part += ' млн рублей'
    }
...
functions[15]['DDL5'] = function () {
    ...
    let ANS = 10000
    while (ANS == 10000) {
        //генерируем задачу, пока не получим вариант не со скучным ответом
        let R1000mas = [
            Big(1010),
            Big(1020),
            Big(1030),
        ...
        let denominatorMas = []
        XMas.forEach(function (value) {
            denominatorMas[denominatorMas.length] = Big(1e6)
                .div(gcdBIG(Big(1e6), Big(1e6).times(value)))
                .toNumber()
        })
        ...
        part += 'В июле '
        let debtStartYear =
            dateTime.getFullYear() + Math.floor(Math.random() * 4)
        part += debtStartYear.toString()
        part += ' года Сара планирует взять кредит в банке на '
        part += N.minus(1).toString()
        if (N.gt(5)) {
        ...
        part +=
            ' в размере S тысяч рублей, где S — целое число. Условия его возврата таковы:\n'
        part += '− каждый январь долг увеличивается на '
        part += R1000.div(1000).minus(1).times(100).toString()
        part += '% по сравнению с концом предыдущего года;\n'
        part +=
        ...
        for (var i = 0; i < N.minus(2).toNumber(); i++) {
            part +=
                ' 0.' + aMas[i].toString() + bMas[i].toString() + '*S' + '    |'
        }
...
functions[15]['RV1Q1'] = function () {
    ...
    if (multiplicator1.gt(0)) {
        let mult = Big(Math.random())
            .times(multiplicator1.plus(1).minus(multiplicator2))
            .plus(multiplicator2)
            .round(0, Big.roundDown)
        if (mult.eq(0)) {
            mult = Big(1)
    ...
    part += dateTime.getFullYear() + Math.floor(Math.random() * 4)
    part +=
        ' года планируется взять кредит в банке на некоторую сумму.\nУсловия его возврата таковы:\n- каждый январь долг возрастает на '
...
functions[15]['RV1Q2'] = function () {
    ...
    part +=
        'Сколько рублей было взято в банке, если известно, что кредит был полностью погашен '
    part += N.toString()
...
functions[15]['RV1Q3'] = function () {
    ...
    part += '% годовых.\n'
    part +=
        'Схема выплаты кредита следующая — 31 декабря каждого следующего года банк начисляет проценты на оставшуюся сумму долга (то есть увеличивает долг на '
    part += R1000.div(10).minus(100).toString()
    part +=
        '%), затем Монечка переводит в банк X рублей. Какой должна быть сумма X, чтобы Монечка выплатил долг '
    part += N.toString()
    part += ' равными платежами (то есть за '
    part += N.toString()
    part += ' года)?'
...
functions[15]['RV1Q5'] = function () {
    ...
    part += dateTime.getFullYear() + Math.floor(Math.random() * 4)
    part +=
        ' года планируется взять кредит на некоторую сумму. Условия возврата таковы:\n'
    ...
    part +=
        'Определите, на какую сумму взяли кредит в банке, если известно, что кредит был выплачен '
    part += N.toString()
    part += ' равными платежами (то есть за '
    part += N.toString()
...
functions[15]['RV2'] = function () {
    ...
    part += S.toString()
    part +=
        ' рублей. Условия возврата кредита таковы: в конце каждого года банк увеличивает текущую сумму долга на '
...
functions[15]['RV3REF'] = function () {
    ...
    while (R1000.eq(Big(1010))) {
        R1000 = R1000mas[Math.floor(Math.random() * R1000mas.length)]
    } // Берем любую ставку кроме первой
    ...
    let ANS = S.times(R1000.div(1000))
        .minus(X)
        .times(R1000.minus(R1000_new).div(1000))
        .toNumber()
    let part: string = ''
    let dateTime = new Date()
    ...
    part += S.toString()
    part +=
        ' рублей. По договору с банком Абрамчик должен был погасить долг двумя равными платежами в феврале '
    ...
    part += (startYear + 1).toString()
    part +=
        ' года Абрамчик сделал первую выплату в соответствии с договором. После этого ему удалось договориться с банком о рефинансировании кредита и уменьшить процент, на который сумма долга вырастет в январе '
    part += (startYear + 2).toString()
    part += ' года, до '
    part += R1000_new.div(10).minus(100).toString()
    part +=
        '%. Какую сумму сэкономит Абрамчик на рефинансировании своего кредита?'
    return [part, ANS]
}
...
functions[15]['RV3MIN'] = function () {
    ...
    if (R1000.gt(1060)) {
        questionType = 0
    } //В этом случае формулируем вопрос про года
    let X = Big(0)
    let Nmas = [Big(3), Big(4), Big(5)]
    while (S.times(1e6).times(R1000.div(1000).minus(1)).gt(X)) {
        //Условие для того, чтобы не оказалось что сумма долга растет бесконечно
    ...
    if (questionType == 1) {
        ...
        part +=
            'На какое минимальное количество месяцев Ицхак Иеремиевич может взять кредит, чтобы ежемесячные выплаты были не более '
        part += X.toString()
        part += ' рублей?'
    }
    ...
    if (questionType == 0) {
        ...
        part += R1000.div(10).minus(100).toString()
        part +=
            '% годовых. Погашение кредита происходит раз в год равными суммами (кроме, может быть, последней) после начисления процентов.\nНа какое минимальное количество лет Сара может взять кредит, чтобы ежегодные выплаты были не более '
        part += X.toString()
        part += ' рублей?'
    }
...
functions[15]['VPRP1'] = function () {
    ...
    for (let i = 0; i < N; i++) {
        Numer = Numer.plus(
            R1000.pow(N.toNumber() - i - 1)
                .times(Big(1000).pow(i + 1))
                .times(prp[i]),
        )
    }
    let gcdNumDen = gcdBIG(Numer, Denom)
    ...
    part += R1000.div(10).minus(100).toString()
    part +=
        '% годовых. Схема выплаты кредита следующая: 1 марта каждого следующего года банк начисляет проценты на оставшуюся сумму долга (то есть увеличивает долг на '
    part += R1000.div(10).minus(100).toString()
    part +=
        '%), затем Йосечка переводит в банк платеж. Весь долг Йосечка выплатил за 3 платежа, причем второй платеж оказался в '
    part += prp[1].toString()
    ...
    part +=
        'больше первого. Сколько рублей взял в кредит Йосечка, если за три года он выплатил банку '
...
functions[15]['VPRP2'] = function () {
    ...
    let prp = [] //Массив, в котором будет храниться пропорция выплат
    if (variant_uslovia == 0) {
        prp = [Big(1), Big(2), Big(4)]
    }
    if (variant_uslovia == 1) {
        prp = [Big(1), Big(3), Big(9)]
    }
    ...
    part += S.toString()
    part +=
        ' рублей. Схема выплата кредита такова: в конце каждого года банк увеличивает на '
    part += R1000.div(10).minus(100).toString()
    part +=
        ' процентов оставшуюся сумму долга, а затем Эфраим переводит в банк свой очередной платеж. Известно, что Эфраим погасил кредит за три года, причем каждый его следующий платеж был ровно '
    if (variant_uslovia == 0) {
        part += 'вдвое'
    }
    if (variant_uslovia == 1) {
    ...
    part +=
        ' больше предыдущего. Какую сумму Эфраим заплатил в первый раз? Ответ дайте в рублях.'
    return [part, ANS]
}
...
functions[15]['VPRP3'] = function () {
    ...
    let prp = [] //Массив, в котором будет храниться пропорция выплат
    if (variant_uslovia == 0) {
        prp = [Big(4), Big(2), Big(1)]
    }
    if (variant_uslovia == 1) {
        prp = [Big(9), Big(3), Big(1)]
    }
    ...
    part += R1000.div(10).minus(100).toString()
    part +=
        ' процентов оставшуюся сумму долга, а затем Бенчик переводит в банк свой очередной платеж. Известно, что Бенчик погасил кредит за три года, причем каждый его следующий платеж был ровно '
    if (variant_uslovia == 0) {
        part += 'вдвое'
    }
    if (variant_uslovia == 1) {
    ...
    part +=
        ' меньше предыдущего. Какую сумму Бенчик заплатил в третий раз? Ответ дайте в рублях.'
